...
async function sbUpsertProduct(product) {
    const { data, error } = await supabase.from('products').upsert(product, { onConflict: 'product_code' }).select();
...
async function sbUpsertManual(manual) {
    const { data, error } = await supabase.from('manuals').upsert(manual).select();
